...
export default function StudentDetailsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 transition-colors duration-300">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[40px] p-10 border border-gray-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row gap-10 items-center">
        <div className="w-32 h-32 bg-purple-100 dark:bg-purple-900/30 rounded-[35px] flex items-center justify-center text-[#5D4291] dark:text-purple-400 text-4xl font-bold uppercase">
        ...
        <div className="space-y-4 text-center md:text-left flex-1">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-4">
            <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-slate-800 rounded-xl text-gray-600 dark:text-gray-300 text-sm">
              <GraduationCap size={18} className="text-[#5D4291] dark:text-purple-400" /> 
              {student.class?.name || "Unassigned"} {/* [SAFEGUARD] */}
            </div>
            <div className="flex items-center gap-2 px-4 py-2 bg-gray-50 dark:bg-slate-800 rounded-xl text-gray-600 dark:text-gray-300 text-sm">
              <User size={18} className="text-[#5D4291] dark:text-purple-400" /> 
              ID: {student.id ? student.id.slice(0, 8) : "N/A"} {/* [FIXED CRASH] */}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[35px] border border-gray-100 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-lg font-bold text-gray-700 dark:text-gray-200">
            Current Grade: {student.class?.name || "Not Enrolled"}
          </p>
          <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">Class ID: {student.classId || "N/A"}</p>
        </div>
      </div>
    </div>
  );
