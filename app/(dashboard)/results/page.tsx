    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500">Manage and view student performance</p>
        </div>
        <button 
              ...
              <tr key={res.id} className="hover:bg-purple-50/20 transition-colors">
                <td className="px-8 py-5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center text-[#5D4291]">
                      <GraduationCap size={16} />
                    </div>
                    <span className="font-bold text-gray-700">{res.student?.name}</span>
                  </div>
                </td>
                <td className="px-8 py-5">
                ...
                <td className="px-8 py-5">
                  <span className={`font-black ${res.marks >= 80 ? 'text-green-500' : 'text-gray-700'}`}>
                    {res.marks}
                  </span>
                </td>
                <td className="px-8 py-5 text-right">
                  <Link href={`/results/${res.id}`} className="text-[#5D4291] font-bold text-sm hover:underline">
                    View Card
                  </Link>
                </td>
              </tr>
          ...
          <div className="bg-white w-full max-w-md rounded-[40px] p-8 shadow-2xl relative animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleAddResult} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-400 uppercase ml-1">Student</label>
                <select 
                  required className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 text-sm"
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-400 uppercase ml-1">Marks Obtained</label>
                <input 
              ...
              <button 
                ...
                className="w-full bg-[#5D4291] text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-[#4a3475] disabled:opacity-70 transition-all mt-4"
              >
