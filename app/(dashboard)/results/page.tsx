...
import { useEffect, useState } from "react";
import Link from "next/link";
import { Trophy, Loader2, Plus, X, GraduationCap } from "lucide-react";
import { authService } from "@/app/services/api.service";
...
export default function ResultsListPage() {
  ...
  const [formData, setFormData] = useState({ studentId: "", subjectId: "", marks: "", examId: "" });

  const fetchData = async () => {
    try {
      setLoading(true);

      const [resData, studentData, subjectData, examData] = await Promise.all([
        authService.getResults(),
        authService.getStudents(),
        authService.getSubjects(),
        authService.getExams ? authService.getExams() : Promise.resolve({ data: { data: [] } })
      ]);
      setResults(resData.data.data || []);
      setStudents(studentData.data.data || []);
      setSubjects(subjectData.data.data || []);
      setExams(examData.data?.data || []);
    } catch (error) {
      console.error("Error fetching data:", error);
    } finally {
      setLoading(false);
  ...
  const handleAddResult = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsModalOpen(false);
      setFormData({ studentId: "", subjectId: "", marks: "", examId: "" });
      fetchData();
      alert("Result added successfully!");
    } catch (error: any) {
      alert(error.response?.data?.message || "Failed to add result");
            ...
            <form onSubmit={handleAddResult} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <select 
                  ...
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 text-sm dark:text-gray-200"
                  onChange={(e) => setFormData({...formData, studentId: e.target.value})}
                >
                  <option value="" className="dark:bg-slate-900">Select Student</option>
                  {students.map((s) => <option key={s.id} value={s.id} className="dark:bg-slate-900">{s.name}</option>)}
              ...
              {/* Dynamic Subject Dropdown [UPDATED] */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase ml-1">Subject</label>
                <select 
                  required 
              ...
              {/* Optional Exam Target Dropdown [ADDED] */}
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase ml-1">Exam Term (Optional)</label>
                <select 
                  value={formData.examId}
                  className="w-full px-5 py-4 bg-gray-50 dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-2xl focus:outline-none focus:ring-2 focus:ring-purple-500/20 text-sm dark:text-gray-200"
                  onChange={(e) => setFormData({...formData, examId: e.target.value})}
                >
                  <option value="" className="dark:bg-slate-900">No Target Exam (General)</option>
                  {exams.map((ex) => (
                    <option key={ex.id} value={ex.id} className="dark:bg-slate-900">{ex.title}</option>
                  ))}
                </select>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase ml-1">Marks Obtained</label>
                <input 
                  type="number" required max="100" min="0" placeholder="e.g. 95"
